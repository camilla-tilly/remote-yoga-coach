...
const post: BlogPost = {
  ...
  content: [
    ...
    { type: 'paragraph', text: 'The instinct is that wellbeing needs a shared physical space. In practice, the live online format removes the frictions that keep people away: nobody has to change clothes or be seen, and colleagues in other time zones can catch the recording. For a remote team it is not a compromise, it is the natural home.' },
    { type: 'heading', text: 'Before the session: the setup' },
    { type: 'paragraph', text: 'Most of the work is in the calendar, not the session:' },
    { type: 'list', items: [
      '**Put it on a recurring invite.** A protected weekly slot beats an ad-hoc one. The habit is the point.',
    ...
    { type: 'callout', text: 'The golden rule: never make it a performance. No pop-quizzes, no "unmute and share how you feel", no forcing cameras on. The lower the threshold, the more of the team you keep.' },
    { type: 'heading', text: 'Getting people to actually show up' },
    { type: 'paragraph', text: 'Attendance is mostly about removing effort. A recurring slot removes the decision. Camera-off removes the self-consciousness. A recording removes the fear of missing out.' },
    { type: 'paragraph', text: 'A visible sponsor helps too: when a manager joins and logs off on time afterwards, it signals the session is genuinely allowed. Low attendance is usually a format problem, not an interest problem. My [remote work burnout guide](/guides/remote-work-burnout) sets out the underlying pattern.' },
    ...
    { type: 'cta', text: 'Would rather not run it yourself? Start with a single pilot session, hosted for you. Once it is a weekly habit, you get attendance and wellbeing reporting.', ctaHref: '/demo', ctaLabel: 'Book a pilot' },
    { type: 'faq', faqItems: [
