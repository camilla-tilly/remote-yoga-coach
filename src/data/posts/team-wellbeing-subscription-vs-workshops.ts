...
const post: BlogPost = {
  ...
  excerpt:
    'A one-off wellbeing workshop gives the team a nice afternoon, while a subscription builds a weekly habit. Here is the honest comparison on results and cost, and when each is the right call.',
  language: 'en',
  ...
  content: [
    { type: 'paragraph', text: 'When a budget opens up for team wellbeing, the instinct is to book a workshop, because it is one simple line item and gives the team a nice afternoon. But the effect often fades within a week. Would a subscription actually change how the team feels?' },
    ...
    { type: 'callout', text: 'A one-off workshop can change how a team feels for an afternoon. A weekly habit can still be changing how the team feels six weeks later. The sessions may look the same, but they do different jobs.' },
    { type: 'heading', text: 'What a subscription changes' },
    { type: 'paragraph', text: 'A subscription is more than a series of workshops, because it changes how the sessions work:' },
    { type: 'list', items: [
      ...
      '**A connected programme.** Sessions build on each other instead of being disconnected pilots.',
    ...
    { type: 'paragraph', text: 'On a per-session sticker price, a one-off can look cheaper. Measured against what actually changes for the team, it rarely is, because the effect fades so quickly. The rough trade-off:' },
    ...
    { type: 'paragraph', text: 'Workshops are not bad. For a one-time occasion, like a team day or an awareness week, a single well-run session is lovely. Just do not expect it to reduce ongoing stress, which needs something that happens regularly. A sensible path is to run a [pilot session](/demo) and convert it into a subscription if it lands.' },
